...
wineries: [
  ...
  { name: "Sola Vine Village", description: "Vineyard resort with wine tours", location: "15km from Nashik" },
...
function isGreeting(input) {
  ...
  return words.some(word =>
    ['hello', 'hi', 'hey', 'namaste', 'नमस्ते'].includes(word)
  );
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //trip plnnner chatbot open via this


 // Auto open Trip Planner from chat
if (containsAny(lowerInput, [
  'trip planner',
  'plan my trip',
  '1 day trip',
  '2 day trip',
  ...
  response = "🧭 Opening Nashik Trip Planner for you...";
  showBotMessage(response);

  setTimeout(() => {
    window.open("trip-planner.html", "_blank");
  }, 1200);
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  ========================= 
  const locationFilter = document.getElementById('reviewLocationFilter');
  locationFilter.addEventListener('change', displayReviews);
  ...
  function displayReviews() {
    ...
    if (reviews.length === 0) {
      reviewsList.innerHTML = `
        <div class="empty-reviews-message">
          <p>No reviews yet for this location.</p>
          <button class="write-review-btn" id="writeFirstReviewInner">
    ...
    reviews.forEach(review => {
      const stars = '★'.repeat(review.rating) + '☆'.repeat(5 - review.rating);
      const visitDate = new Date(review.date).toLocaleDateString();
      ...
      card.innerHTML = `
        ...
        <h4>${review.title}</h4>
        <p>${review.text}</p>
        <small>${review.name} • ${visitDate}</small>
      `;
      reviewsList.appendChild(card);
    });
  }
  ...
  ========================= 
  document.addEventListener('click', function (e) {
    if (e.target && e.target.id === 'writeFirstReviewInner') {
...
function handleHelpline(value) {
  if (!value) return;

  if (value === "nearest-hospital") {
    callNearestHospital();   // 👈 direct function call
  } else {
    window.location.href = value; // tel:
  }
}

/* Nashik hospitals */
const hospitals = [
  { name: "Civil Hospital", lat: 19.9975, lon: 73.7898, phone: "02532574266" },
  { name: "Six Sigma Hospital", lat: 20.0114, lon: 73.8077, phone: "02536600000" },
  { name: "Apollo Hospital", lat: 20.0056, lon: 73.7932, phone: "02536620000" },
  { name: "Suyash Hospital", lat: 19.9640, lon: 73.7736, phone: "02532577000" }
];
...
function callNearestHospital() {
  ...
  navigator.geolocation.getCurrentPosition(pos => {
    ...
    let min = distance(uLat, uLon, nearest.lat, nearest.lon);

    hospitals.forEach(h => {
      let d = distance(uLat, uLon, h.lat, h.lon);
      if (d < min) {
        min = d;
        nearest = h;
      }
    ...
    alert("Location allow kara 🙏");
  });
}

//topup arrow from bottom
